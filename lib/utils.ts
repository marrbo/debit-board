...
export const isContainer =
  (process.env.RUNNING_IN_CONTAINER ?? false) === "true";
export const isLocalHost =
  process.env.NODE_ENV === "development" && !isContainer;
...
export function getNextAuthUrl() {
  return isLocalHost
    ? process.env.NEXTAUTH_URL_LOCAL
    : process.env.NEXTAUTH_URL;
}

export function parseRepoName(repo: string): {
  gerencia: string;
  nucleo: string;
} {
...
export function groupBy<T>(array: T[], key: keyof T): Record<string, T[]> {
  return array.reduce(
    (acc, item) => {
      ...
      const groupKey =
        typeof item[key] === "object" ? `Sem ${keyStr}` : String(item[key]);
