...
export default function SavedQueriesClient({ initialQueries }: Props) {
  const router = useRouter();
  const [queries, setQueries] = useState<SavedQueryItem[]>(initialQueries);
  ...
  // Atualiza a lista após mutações, sem useEffect para busca inicial
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingId
        ? { id: editingId, name, queryString, context, visibility }
        : { name, queryString, context, visibility };
          ...
          <form onSubmit={handleSave} className="bg-white dark:bg-apple-card-dark rounded-2xl max-w-lg w-full shadow-2xl p-6 space-y-4 border border-apple-border-light dark:border-apple-border-dark">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={context} onChange={(e) => setContext(e.target.value)} className="w-full">
              ...
              <div>
                ...
                <select value={visibility} onChange={(e) => handleVisibilityChange(e.target.value)} className="w-full">
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-apple-border-light dark:border-apple-border-dark">
              ...
              <button type="submit" className="px-4 py-2 rounded-xl text-sm font-medium bg-apple-blue text-white">
                Salvar
              </button>
            </div>
          </form>
...
// Função auxiliar para rótulo de visibilidade
