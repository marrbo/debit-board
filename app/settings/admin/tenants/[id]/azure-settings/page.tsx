...
export default async function TenantAzureSettings(props: {
  ...
  if (!tenant) {
    return (
      <div className="w-full mx-auto py-8">
        <div className="bg-apple-red/10 border border-apple-red/40 rounded-lg p-6 text-error shadow-sm hover:drop-shadow-lg">
          <h2 className="text-xl font-bold text-heading dark:text-heading mb-2">
          ...
          <a
            href="/settings/admin"
            className="mt-4 inline-block bg-apple-tertiary-light/20 dark:bg-apple-tertiary-dark/20 hover:bg-page dark:hover:bg-surface px-4 py-2 rounded-lg text-heading dark:text-heading text-sm border border-default dark:border-strong transition-colors"
          >
            Voltar para Admin
          </a>
        </div>
    ...
    <div className="w-full mx-auto">
      ...
      <div className="bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg p-6 shadow-sm hover:drop-shadow-lg transition-colors">
        ...
        <p className="text-sm text-muted dark:text-muted mb-6">
          Essas configurações serão utilizadas por todos os usuários vinculados
          a este Tenant.
        </p>
        ...
        <form action={updateAzureSettings} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="url"
                name="instanceUrl"
                defaultValue={tenant.azureSettings?.instanceUrl || ""}
            ...
            <div>
              ...
              <input
                ...
                defaultValue={tenant.azureSettings?.azureCollection || ""}
                className="w-full bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg px-4 py-2 text-heading dark:text-heading focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors"
                required
              />
            </div>
          ...
          <div>
            ...
            <input
              type="password"
              name="pat"
              defaultValue={tenant.azureSettings?.pat || ""}
              className="w-full bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg px-4 py-2 text-heading dark:text-heading focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                name="username"
                defaultValue={tenant.azureSettings?.username || ""}
                className="w-full bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg px-4 py-2 text-heading dark:text-heading focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                name="defaultProject"
                defaultValue={tenant.azureSettings?.defaultProject || ""}
                className="w-full bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg px-4 py-2 text-heading dark:text-heading focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              name="reportTitle"
              defaultValue={tenant.azureSettings?.reportTitle || ""}
              className="w-full bg-surface dark:bg-surface border border-default dark:border-strong rounded-lg px-4 py-2 text-heading dark:text-heading focus:outline-none focus:ring-2 focus:ring-brand/30 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-3 py-2">
            <input
              ...
              defaultChecked={tenant.azureSettings?.ignoreTlsErrors || false}
            ...
            />
            <label
              htmlFor="ignoreTlsErrors"
              className="text-sm text-body dark:text-body cursor-pointer select-none transition-colors"
            >
          ...
          <div className="flex justify-end pt-4 border-t border-default dark:border-strong">
            <button
              type="submit"
              className="bg-brand hover:bg-brand/80 text-white px-5 py-2 rounded-lg text-sm font-medium transition-all shadow-sm hover:drop-shadow-lg"
            >
