...
| `status` | Status atual do ciclo de vida da Issue. | `open`, `fixed`, `recurring`, `wont_fix`, `unresolved`, `resolved` | `status:open` |
| `is` | Filtros de estado especiais para Observations. | `unresolved` | `is:unresolved` `is:resolved` |
...
### O Curinga `*`
O asterisco `*` é extremamente útil para buscas abrangentes. Ele pode ser usado no início, no meio e/ou no final de um termo de busca.
