...
const contextPropertiesMapping: Record<
  string,
  { prop: string; desc: string; ex: string }[]
> = {
  observations: [
    {
      prop: "category",
      desc: "Categoria da vulnerabilidade",
      ex: 'category:"Broken Access Control"',
    },
    {
      prop: "severity",
      desc: "Severidade (critical, high, medium, low)",
    ...
    {
      prop: "status",
      desc: "Status da issue (open, resolved, etc.)",
      ex: "status:open",
    },
...
const operators = [
  ...
  { op: "!", desc: "Nega um termo (atalho)", ex: "!branch:main" },
  {
    op: "( )",
    desc: "Agrupamento",
    ex: "(severity:high OR severity:critical)",
  },
  { op: "*", desc: "Curinga (Wildcard)", ex: "fileName:*Controller.cs" },
];

export default function DBQLHelpModal({
  isOpen,
  onClose,
  ...
  const currentProperties =
    contextPropertiesMapping[context] || defaultProperties;
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-white dark:bg-[#1C1C1E] w-full max-w-4xl rounded-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
      >
        {/* Header do Modal */}
            ...
            <div className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-gray-100 dark:bg-gray-800">
                  <tr>
                    <th className="p-3 font-semibold text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700 w-1/4">
                      Propriedade
                    </th>
                    <th className="p-3 font-semibold text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700 w-1/3">
                      Descrição
                    </th>
                    <th className="p-3 font-semibold text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700 w-auto">
                      Exemplo
                    </th>
                  </tr>
                </thead>
        ...
        <div className="p-5 bg-white dark:bg-[#1C1C1E] border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center gap-4 justify-between">
          <span className="text-sm text-gray-500 dark:text-gray-400">
            Para uma documentação completa com exemplos avançados, consulte a
            Wiki.
          </span>
          <Link
