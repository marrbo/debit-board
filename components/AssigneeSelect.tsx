...
export default function AssigneeSelect({
  ...
  const getAvatarUrl = (user?: IUser) => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(
      name,
    )}&length=2&background=0D8ABC&color=fff&width=28&height=28`;
  };

  return (
    <div
      ref={containerRef}
      className={`flex itens-center justify-between ${className}`}
    >
      {/* BOTÃO GATILHO: */}
      <button
        type="button"
        onClick={handleToggle}
        className={`group flex itens-center focus:outline-none ${!selectedUser ? "p-2 border rounded-full border-muted border-dotted p-2" : ""}`}
      >
        {selectedUser ? (
          <Image
            ...
            width={48}
            height={48}
            className="w-7 h-7 rounded-full border-transparent group-hover:border-brand transition-all object-cover"
          />
        ) : (
      ...
      {isOpen &&
        createPortal(
          <div
            className="fixed z-[9999] w-64 bg-white dark:bg-[#1C1C1E] border border-default dark:border-strong rounded-lg shadow-2xl overflow-hidden"
            style={{
              top: `${coords.top}px`,
              left: `${coords.left}px`,
              position: "absolute",
