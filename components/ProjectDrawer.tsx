...
export default function ProjectDrawer({
  ...
  const projectImage =
    ...
    `https://ui-avatars.com/api/?name=${encodeURIComponent(
      name,
    )}&length=3&background=0D8ABC&color=fff&width=48&height=48`;

  return (
    <div className="fixed inset-0 -top-6 z-[9999] flex justify-end">
      {/* Overlay */}
      <div
        className="absolute inset-0 -top-6 bg-black/40 backdrop-blur-sm"
        onClick={onClose}
      />
        ...
        <div className="flex items-start justify-between border-b border-default dark:border-strong pb-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12px] text-muted mt-1 font-mono text-ellipsis text-wrap">
                {url}
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-0 text-muted hover:text-error transition-colors"
            aria-label="Fechar"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono">
                  {syncDate ? new Date(syncDate).toLocaleDateString() : "Nunca"}
                </p>
              </div>
              <div>
                <span className="text-[10px] uppercase text-muted">
